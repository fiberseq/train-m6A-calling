...
def m6AGenerator(
    data_path, random_state=None, pin_memory=True, num_workers=0, batch_size=32
):
    """
    ...
    print(
        f"Training features shape {X_train.shape}, training labels shape: {y_train.shape}"
    )
    print(
        f"Validation features shape {X_val.shape}, validation labels shape: {y_val.shape}"
    )

    # Get the training data generator
    X_gen = DataGenerator(X_train, y_train, random_state=random_state)
            ...
            for data in training_data:
                ...
                if verbose and iteration % validation_iter == 0:
                    with torch.no_grad():
                        ...
                        print(
                            f"Epoch {epoch}, iteration {iteration}, train loss: {(avg_train_loss / avg_train_iter):4.4f}, validation loss: {valid_loss:4.4f}"
                        )

                        print(
                            f"Validation iteration {iteration}, AUPR: {sklearn_ap}, Accuracy: {sklearn_acc}, AUROC: {sklearn_rocauc}"
                        )
